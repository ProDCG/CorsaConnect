...
class ACServerManager:
    ...
    def start_server(
        ...
        if not cars:
            ...
            try:
                cars = [
                    d for d in sorted(os.listdir(ac_content_cars)) if os.path.isdir(os.path.join(ac_content_cars, d))
                ]
        ...
        for car_id in cars:
            ...
            elif not os.path.isdir(os.path.join(car_dir, "data")) and not os.path.isfile(
                os.path.join(car_dir, "data.acd")
            ):
                rejected_cars.append({"id": car_id, "reason": "missing data/ folder and data.acd (car is incomplete)"})
                logger.warning(
                    "CAR REJECTED: '%s' — missing data/ and data.acd in %s (incomplete car)", car_id, car_dir
                )
            else:
        ...
        try:
            ...
            logger.info(
                "AC server started for '%s' on port %d (PID: %d) — still alive after 3s ✓",
                group_name,
                udp_port,
                proc.pid,
            )
    ...
    def _sync_server_content(self, ac_server_dir: str, cars: list[str], track: str, enable_csp: bool = False) -> None:
    ...
    def _write_server_cfg(
        ...
        if not cars:
            return None  # Cannot write server config without any cars
        car_str = ";".join(cars)
        ...
        cfg += (
            ...
            f"\n"
            f"[WEATHER_0]\n"
            f"GRAPHICS=sol_42_thunderstorm_type={w_id}_time={fx_time}\n"
            f"__CM_GRAPHICS=sol_42_thunderstorm\n"
        ...
        if write_to_disk:
            ...
            logger.info(
                "Wrote server_cfg.ini and welcome.txt: track=%s cars=%s max_clients=%d port=%d",
                track,
                car_str,
                max_clients,
                udp_port,
    ...
    def _write_entry_list(
        ...
        logger.info(
            ...
            len(rig_ids),
            ai_count,
            max(0, idx - len(rig_ids) - ai_count),
            idx,
        )
